...
def presMat(orig_mat, rot_mat, ins = True):
	rect = visual.Rect(
		win = win, 
		units = "pix",
		width = 60,
...
def trainBuffer(exp):
    if exp == 1:
        ...
        In this task, we are going to show you two grids.  (picture here).  Sometimes the grids are the same object, but one is rotated from the other.  (picture here).  Sometimes, they are different objects.  Your job is to decide whether the grids are the same object or not.  If they are, press 'M' for match.  Otherwise, press 'X' for mismatch.
