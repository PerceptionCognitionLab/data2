...
mask=visual.TextStim(window,text="#",pos = (0,0))
...
for t in range(pracN):				 
	out=doTrial(pracCond[t],35,10,10,10)
...
for t in range(N):
	...
	if trl==0 and blk>0:
		...
		window.flip()
		event.waitKeys()				 
	out=doTrial(cond[t],fp[n], CueTime, blanktimeC, blanktimeI)
