...
######### Specifics: No masks and line lengths jitter
#########            randomly along the X and Y axes
####################################################
...
rate = 1.3    #Increment rate
nLen = 9      #Number of line lengths included
...
## Set of possible positions for the lines (x axis)
numHPos = 3
w = list(range(numHPos-1))
hJump = 50
hCPos1 = np.array(w)*hJump
hCPos2 = -(hCPos1)
hCPos = list(hCPos1) + list(hCPos2)
...
def trialAbsId(endpoint,horJitter,map):
    box=visual.Rect(window,width=wBox,height=hBox)
    vertPos = random.sample(list(vCPos),1)
    im=visual.Line(window, start=[-endpoint+horJitter,np.array(vertPos)],
                   end=[endpoint+horJitter,np.array(vertPos)])
    feedback=visual.TextStim(window,text=map,
                        pos=(horJitter,np.array(vertPos)+50),
                        height=20,bold=True,
                        anchorVert="bottom")
    blank=visual.TextStim(window,"")
    timer.reset()
...
for n in range(numTrials):
    ...
    if t==0 and b>0:
        takeABreak()
    out=trialAbsId(endpoint[stimIndex[n]],horJitter[n],map[stimIndex[n]])
