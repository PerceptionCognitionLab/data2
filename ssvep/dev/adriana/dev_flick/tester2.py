...
Welcome=visual.TextStim(window, text = "Identify the central letter\n Press 'q' for A and 'p' for H key\n\n\n Press any key to begin", pos = (0,0))
Text=visual.TextStim(window, text = "Press any key to continue to next trial", pos = (0,0))
Reminder=visual.TextStim(window, text = "Press 'q' for A or 'p' for H", pos = (0,0))
Quit=visual.TextStim(window, text = "Quiting experiment...", pos = (0,200))
Thnx=visual.TextStim(window, text = "Thank you for your time! You may call the experimenter now!", pos = (0,0))
...
for trial in range(4):
        ...
        [resp,RT,frameFlag]=doTrial(flanker,target,crit[trial])
        rt = decimal.Decimal(RT).quantize(decimal.Decimal('1e-3'))
        addData = (sessionID, trial,target,flanker,resp,rt,crit[trial],frameFlag)
        if useDB:
